#!/usr/bin/env python3
"""
orb_montecarlo.py – Monte-Carlo-Simulation für ORB Bot.

Lokale Kopie von Trading_Bot/vbt_montecarlo.py, angepasst:
  - Side-Filter erweitert: SELL, COVER, PARTIAL_SELL
  - Output-Prefix "orb_" statt "v7_"

Drei Methoden:
  1. Trade-Reshuffling    – zufällige Reihenfolge
  2. Bootstrap-Sampling   – mit Zurücklegen
  3. Noise-Injection      – ± Gaussian-Noise

Nutzung:
    from orb_montecarlo import MonteCarlo
    ...
    mc.summary()
    mc.plot_distributions(output_dir=Path("orb_trading_data"))
"""
...
def _equity_from_returns(returns: np.ndarray, capital: float) -> np.ndarray:
    """Erstelle Equity-Kurve aus Trade-PnL-Beträgen."""
...
def _metrics_from_equity(eq: np.ndarray) -> Dict[str, float]:
    """Kennzahlen aus einer Equity-Kurve."""
    if len(eq) < 2 or eq[0] <= 0:
        return {"cagr": 0.0, "sharpe": 0.0, "max_dd": 0.0, "profit_factor": 0.0}

    n_trades = len(eq) - 1
    years    = max(n_trades / 250, 1e-6)
...
class MonteCarlo:
    ...
    Parameter
    ---------
    trades_df       : Trade-DataFrame (muss ``pnl``-Spalte enthalten)
    initial_capital : Startkapital
    """

    # Erweitert gegenüber v7: COVER für Short-Exits
    _EXIT_SIDES = {"SELL", "COVER", "PARTIAL_SELL"}
    ...
    def __init__(self, trades_df: pd.DataFrame, initial_capital: float = 10_000.0):
        ...
        mask = trades_df["side"].isin(self._EXIT_SIDES)
    ...
    def run_reshuffling(self, n: int = 2000, seed: Optional[int] = None) -> None:
        """Trade-PnLs in zufälliger Reihenfolge."""
    ...
    def run_bootstrap(self, n: int = 2000, seed: Optional[int] = None) -> None:
        """Bootstrap-Sampling mit Zurücklegen."""
        rng   = np.random.default_rng(seed)
        n_obs = len(self._pnl)
        results = []
        for _ in range(n):
    ...
    def summary(self) -> pd.DataFrame:
        """Drucke Median + 5./95. Perzentil und gib DataFrame zurück."""
    ...
    def plot_distributions(
        ...
        """Histogramme für alle vier Kennzahlen."""
        ...
        for method in active_methods:
            ...
            for ax, (col, label, unit_note, mult) in zip(axes, metrics):
                ...
                vals = df[col] * mult
                try:
                    ax.hist(vals, bins=60, alpha=0.75, color="steelblue",
                            edgecolor="None")
                except ValueError:
                    ax.hist(vals, bins="auto", alpha=0.75, color="steelblue",
                            edgecolor="None")
                ax.axvline(vals.median(), color="red", linewidth=1.5,
                           label=f"Median {vals.median():.2f}")
                ax.axvline(vals.quantile(0.05), color="orange", linewidth=1.0,
                           linestyle="--", label="5. Pzl.")
                ax.axvline(vals.quantile(0.95), color="green", linewidth=1.0,
                           linestyle="--", label="95. Pzl.")
                ax.set_title(f"{label}{' (' + unit_note + ')' if unit_note else ''}")
                ax.legend(fontsize=8)
                ax.grid(True, alpha=0.3)

            plt.tight_layout()
            fname = output_dir / f"orb_mc_{method}.png"
    ...
    def export_csv(self, output_dir: Optional[Path] = None) -> None:
        """Exportiere Simulations-Ergebnisse als CSV."""
        if output_dir is None:
            output_dir = Path(__file__).parent / "orb_trading_data"
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for method, results in self._results.items():
            if not results:
                continue
            df   = pd.DataFrame(results)
            path = output_dir / f"orb_mc_{method}.csv"
