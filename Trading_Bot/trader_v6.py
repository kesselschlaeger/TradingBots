...
CONFIG = {
    ...
    "lower_timeframe": "15Min",           # "5Min" oder "15Min"
    ...
    "sma_short": 20,
    "sma_long":  30,          # war 50 → jetzt 30 nach WFR Simulation,
    # --- Fast Cross (EMA9/21 oder SMA10/30 – reagiert schneller) --------
    "use_fast_cross":         True,   # EMA/SMA-Cross als zusätzliches Signal
    "fast_cross_type":        "EMA",  # "EMA" oder "SMA"
    "fast_cross_short":       9,      # z.B. EMA9 oder SMA10
    "fast_cross_long":        21,     # z.B. EMA21 oder SMA30
    # --- Early Golden Cross (Pre-Cross-Filter) --------------------------
    "use_early_golden_cross": True,   # Einstieg bevor SMA20 SMA50 kreuzt
    "early_gc_proximity_pct": 0.02,   # SMA20 max. 2 % unter SMA50
    "early_gc_rsi_min":       55,     # RSI muss stark bullisch sein
    # --- Pullback-Entry nach Golden/Fast Cross -------------------------
    "use_pullback_entry_daily": True,  # Warte auf Pullback zu SMA20/EMA nach Cross
    "pullback_daily_lookback":  15,    # Cross muss in letzten N Bars liegen
    "pullback_daily_ema":       20,    # EMA-Periode als Pullback-Ziel
    "pullback_daily_proximity": 0.015, # Bar-Low max. 1.5 % über SMA/EMA
    "pullback_daily_rsi_min":   50,    # RSI-Minimum für Umkehrbestätigung
    # --- Bestätigungsfilter --------------------------------------------
    "use_rsi_filter":    True,
    "rsi_buy_min":       50,          # war 40 → jetzt 50 nach WFR Simulation,   # Mindestmomentum vorhanden
    ...
    # --- Trend Filter ---------------------------------------------------
    "adx_period":    14,
    "adx_threshold": 15,          # war 20 → jetzt 15 nach WFR Simulation ,
    # --- VIX Regime Filter ---------------------------------------------
    "use_vix_filter":     True,
    ...
    "daily_event_log_dir": Path(__file__).parent / "trading_data" / "daily_logs",
    "daily_summary_once_per_day": True,
    "daily_summary_state_file": Path(__file__).parent / "trading_data" / "daily_summary_state.json",
    # Tageszusammenfassung erst ab dieser ET-Uhrzeit senden (z.B. 15:55 = kurz vor Close)
    "daily_summary_send_after_et": "15:55",
}
...
class AlpacaClient:
    ...
    def fetch_intraday_bars(self, symbol: str, days: int = 10,
                            tf: str = "15Min") -> pd.DataFrame:
        """Intraday-Bars (5Min oder 15Min) für den Lower-Timeframe-Filter."""
        try:
            if tf.endswith("Min"):
                minutes   = int(tf.replace("Min", ""))
                timeframe = TimeFrame(minutes, "Minute")
            else:
                timeframe = TimeFrame.Day
            req = StockBarsRequest(
                symbol_or_symbols=symbol,
            ...
            print(f"[Alpaca] Intraday-Fehler {symbol} ({tf}): {e}")
            return pd.DataFrame()
    ...
    def get_position_orders(self, symbol: str) -> dict:
        """
        Gibt die offenen SL- und TP-Legs einer Bracket-Order zurück.
        Rückgabe: {'stop': {'id': ..., 'stop_price': ...},
                   'tp':   {'id': ..., 'limit_price': ...}}
        """
        try:
            req    = GetOrdersRequest(status="open", symbols=[symbol], limit=20)
            orders = self.trading.get_orders(req)
            result = {}
            for o in orders:
                if o.side.value == "sell":
                    if o.type == OrderType.STOP:
                        result["stop"] = {"id": str(o.id),
                                          "stop_price": float(o.stop_price or 0)}
                    elif o.type == OrderType.LIMIT:
                        result["tp"] = {"id": str(o.id),
                                        "limit_price": float(o.limit_price or 0)}
            return result
        except Exception as e:
            print(f"[Alpaca] Position-Orders {symbol} Fehler: {e}")
            return {}

    def update_stop(self, order_id: str, new_stop_price: float) -> bool:
        """Hebt den Stop-Loss einer offenen Order an (replace_order)."""
        try:
            req = ReplaceOrderRequest(stop_price=round(new_stop_price, 2))
            self.trading.replace_order_by_id(order_id, req)
            print(f"[Alpaca] Stop-Loss → {new_stop_price:.2f}")
            return True
        except Exception as e:
            print(f"[Alpaca] Stop-Update Fehler: {e}")
            return False

    def update_take_profit(self, order_id: str, new_limit_price: float) -> bool:
        """Ändert den Take-Profit einer offenen Order (replace_order)."""
        try:
            req = ReplaceOrderRequest(limit_price=round(new_limit_price, 2))
            self.trading.replace_order_by_id(order_id, req)
            print(f"[Alpaca] Take-Profit → {new_limit_price:.2f}")
            return True
        except Exception as e:
            print(f"[Alpaca] TP-Update Fehler: {e}")
            return False
...
def compute_indicators(df: pd.DataFrame, cfg: dict = None) -> pd.DataFrame:
    ...
    # Fast Cross MA (EMA9/21 oder SMA10/30 für schnellere Signale)
...
def send_telegram(message: str) -> bool:
    """Sendet eine Telegram-Nachricht via ~/.secrets/telegram.token + telegram.chat_id."""
    try:
...
class Strategy:
    ...
    def signal_row(self, df: pd.DataFrame) -> Tuple[str, str]:
        ...
        # ---- Fast EMA/SMA Cross (EMA9/21 od. SMA10/30 → früher als GC) ----
        if self.cfg.get("use_fast_cross", False) and "FC_FAST" in df.columns:
            ...
            if (pd.notna(fc_pf) and pd.notna(fc_ps) and pd.notna(fc_lf) and pd.notna(fc_ls)
                    and float(fc_pf) <= float(fc_ps) and float(fc_lf) > float(fc_ls)):
                rsi = last["RSI"]
                rsi_ok = (pd.notna(rsi)
                          and self.cfg["rsi_buy_min"] <= float(rsi) <= self.cfg["rsi_buy_max"])
                macd_ok = pd.notna(last["MACD_hist"]) and float(last["MACD_hist"]) > 0
                vol_ok = (not self.cfg.get("use_volume_filter", True)
                          or (pd.notna(last["Vol_SMA"]) and last["Volume"] > last["Vol_SMA"]))
                if rsi_ok and macd_ok and vol_ok:
                    ft = self.cfg.get("fast_cross_type", "EMA")
                    fs = self.cfg.get("fast_cross_short", 9)
                    fl = self.cfg.get("fast_cross_long", 21)
                    return "BUY", f"Fast {ft} Cross ({fs}/{fl}) | RSI={float(rsi):.0f}"

        # ---- Early Golden Cross (Pre-Cross: SMA20 nähert sich SMA50) ----
        if self.cfg.get("use_early_golden_cross", False):
            s20 = last["SMA20"]; s50 = last["SMA50"]
            p20 = prev["SMA20"]; p50 = prev["SMA50"]
            if pd.notna(s20) and pd.notna(s50) and float(s20) < float(s50):
                sma_dist = (float(s50) - float(s20)) / (float(s50) + 1e-9)
                approaching = sma_dist <= self.cfg.get("early_gc_proximity_pct", 0.02)
                # SMA20 steigt schneller als SMA50 → konvergiert
                converging = (pd.notna(p20) and pd.notna(p50)
                              and (float(s20) - float(p20)) > (float(s50) - float(p50)))
                rsi = last["RSI"]
                rsi_ok = pd.notna(rsi) and float(rsi) >= self.cfg.get("early_gc_rsi_min", 55)
                ...
                if approaching and converging and rsi_ok and macd_strong and vol_ok:
                    return "BUY", (f"Early Golden Cross | SMA20 {sma_dist:.1%} unter SMA50"
                                   f" | RSI={float(rsi):.0f}")
        ...
        # ---- Pullback-Entry nach Golden/Fast Cross ----
        if self.cfg.get("use_pullback_entry_daily", False):
            pb_sig, pb_reason = self.daily_pullback_signal(df)
            if pb_sig == "BUY":
                return "BUY", pb_reason
    ...
    # ------------------------------------------------------------------
    def _recent_cross(self, df: pd.DataFrame, lookback: int) -> bool:
        ...
        for i in range(1, len(window)):
            ...
            c = window.iloc[i]
            # Golden Cross
            if (pd.notna(p["SMA20"]) and pd.notna(p["SMA50"])
                    and pd.notna(c["SMA20"]) and pd.notna(c["SMA50"])):
                if float(p["SMA20"]) <= float(p["SMA50"]) and float(c["SMA20"]) > float(c["SMA50"]):
                    return True
            # Fast Cross
    ...
    def daily_pullback_signal(self, df: pd.DataFrame) -> Tuple[str, str]:
        """
        Pullback-Entry nach Golden/Fast Cross (Daily).

        Bedingungen:
          1. Ein GC oder Fast Cross fand innerhalb der letzten N Bars statt.
          2. SMA20 > SMA50 (Aufwärtstrend intakt).
          3. Bar-Low touchierte SMA20 oder EMA<pullback_daily_ema>.
          4. Bullische Umkehrkerze (Close > Open) oder Erholung (Close > prev Close).
          5. RSI > pullback_daily_rsi_min.
          6. MACD-Histogramm > 0 (sofern use_macd_filter aktiv).
          7. Volume > Vol_SMA (sofern use_volume_filter aktiv).
        """
        cfg = self.cfg
        if len(df) < 3:
            return "HOLD", ""
        ...
        if not self._recent_cross(df, lookback):
            return "HOLD", ""

        last = df.iloc[-1]
        prev = df.iloc[-2]

        # Aufwärtstrend muss nach wie vor intakt sein
        ...
        ema_d = float(df["Close"].ewm(span=ema_p, adjust=False).mean().iloc[-1])

        touched_sma = float(last["Low"]) <= sma20 * (1 + prox)
        touched_ema = float(last["Low"]) <= ema_d * (1 + prox)
        ...
        if pd.isna(rsi) or float(rsi) < rsi_min:
            return "HOLD", ""
        ...
        ref = "SMA20" if touched_sma else f"EMA{ema_p}"
        return "BUY", f"Pullback-Entry → {ref} | RSI={float(rsi):.0f}"
    ...
    def intraday_entry_confirmation(self, df_lower: pd.DataFrame,
                                    cfg: dict) -> Tuple[bool, str]:
        """
        Lower-Timeframe-Filter (5-Min / 15-Min).
        Gibt (ok, Begründung) zurück.

        Bedingungen (alle drei müssen erfüllt sein):
          1. Pullback: Preis liegt max. 1.5 % von der EMA<lower_ema_period> entfernt
          2. Momentum: RSI(14) > lower_rsi_min
          3. Trigger:  MACD-Histogramm > 0 UND steigend
                       ODER Preis bricht über das Hoch der letzten 3–5 Kerzen
        """
        if len(df_lower) < 30:
        ...
        # EMA auf Lower TF
        ema_p = cfg.get("lower_ema_period", 20)
        df["EMA_LTF"] = df["Close"].ewm(span=ema_p, adjust=False).mean()

        last = df.iloc[-1]
        prev = df.iloc[-2]

        # 1) Pullback zur EMA
        near_ema = (abs(last["Close"] - last["EMA_LTF"])
                    / (last["EMA_LTF"] + 1e-9)) < 0.015

        # 2) RSI-Momentum (RSI aus compute_indicators bereits vorhanden)
        rsi = last.get("RSI", float("nan"))
        momentum_ok = pd.notna(rsi) and float(rsi) > cfg.get("lower_rsi_min", 50)
        ...
        # 3b) Breakout über Hoch der letzten 5 Kerzen
        ...
        if cfg.get("pullback_entry", True):
            if near_ema and momentum_ok and (macd_ok or breakout_ok):
                parts = [f"Pullback EMA{ema_p}", f"RSI {float(rsi):.1f}"]
                if macd_ok:     parts.append("MACD↑")
                if breakout_ok: parts.append(f"Breakout >{recent_high:.2f}")
                return True, " | ".join(parts)
        else:
            if momentum_ok and last["Close"] > last["EMA_LTF"] and (macd_ok or breakout_ok):
                trigger = "MACD↑" if macd_ok else f"Breakout >{recent_high:.2f}"
                return True, f"Momentum RSI {float(rsi):.1f} | {trigger}"

        return False, "kein Lower-TF-Setup"

    def daily_mtf_proxy(self, sub: pd.DataFrame, cfg: dict) -> Tuple[bool, str]:
        """
        Simuliert den Lower-Timeframe-Filter auf Basis von Daily-Bars (Backtest-Proxy).

        Unterschied zu intraday_entry_confirmation:
          - Pullback: Bar-Low <= EMA<n> * 1.015  (Preis touchierte EMA intraday)
          - RSI/MACD/Breakout: identische Logik wie beim Live-Filter
        """
        if len(sub) < 30:
            return False, "zu wenige Daily-Daten für MTF-Proxy"

        df = sub.copy()
        ema_p = cfg.get("lower_ema_period", 20)
        df["EMA_PROXY"] = df["Close"].ewm(span=ema_p, adjust=False).mean()

        last = df.iloc[-1]
        prev = df.iloc[-2]

        # 1) Pullback-Proxy: Low touchierte die EMA während der Tageskerze
        ema_val = last["EMA_PROXY"]
        touched_ema = last["Low"] <= ema_val * 1.015
        ...
        # 3b) Breakout über Hoch der letzten 5 Tageskerzen
        recent_high = df["High"].iloc[-6:-1].max()
        breakout_ok = last["Close"] > recent_high

        if cfg.get("pullback_entry", True):
            if touched_ema and momentum_ok and (macd_ok or breakout_ok):
                parts = [f"Pullback EMA{ema_p} (Daily)", f"RSI {float(rsi):.1f}"]
                if macd_ok:     parts.append("MACD↑")
                if breakout_ok: parts.append(f"Breakout >{recent_high:.2f}")
                return True, " | ".join(parts)
        else:
            if momentum_ok and last["Close"] > ema_val and (macd_ok or breakout_ok):
                trigger = "MACD↑" if macd_ok else f"Breakout >{recent_high:.2f}"
                return True, f"Momentum RSI {float(rsi):.1f} | {trigger}"

        return False, "kein MTF-Proxy-Setup"
...
class Backtester:
    ...
    def _run_single_backtest(self, data_dict: Dict[str, pd.DataFrame], symbols: List[str],
        ...
        for date in all_dates:
            ...
            if is_portfolio:
                ...
                dd_active = (peak - equity) / peak >= self.cfg.get("max_drawdown_pct", 0.15)
            ...
            if not dd_active:
                ...
                for sym in symbols:
                    ...
                    if i_loc < self.cfg["sma_long"] + 20: continue

                    sub = df.iloc[:i_loc + 1]
                    signal, reason = self.strat.signal_row(sub)
                    if signal not in ("BUY", "BUY_MR"): continue

                    # ── MTF-Proxy-Filter (Daily-Simulation) ────────────────────
                    if self.cfg.get("use_multi_timeframe", False) and signal == "BUY":
                        mtf_ok, mtf_reason = self.strat.daily_mtf_proxy(sub, self.cfg)
                        if not mtf_ok:
                            continue
                        reason += f" | {mtf_reason}"
                    # ──────────────────────────────────────────────────────────
                    ...
                    if self.cfg.get("use_ml", False):
                        ...
                        if hasattr(self.strat, 'ml_prob') and self.strat.ml_prob and \
                           self.strat.ml_prob(feats) < self.cfg.get("ml_prob_threshold", 0.6):
                            continue
                    ...
                    positions[sym] = {
                        ...
                        "highest": entry_price,
                        "stop": entry_price - self.cfg["initial_sl_atr_mult"] * atr_val,
                        "trailing": entry_price - self.cfg["trailing_atr_mult"] * atr_val,
                        "partial": False,
                        "strategy": "mr" if signal == "BUY_MR" else "trend",
                        "mr_target": mr_target,
                    }
    ...
    def _print_asset_table(self, asset_results: dict, portfolio_result: dict):
        ...
        for sym, res in asset_results.items():
            print(f"{sym:<6} {res['total_trades']:>6} {res['first_trade_date']:>10} "
                  f"{res['last_trade_date']:>10} {res['total_return_pct']:>+10.2f} "
                  f"{res['bnh_return_pct']:>+8.2f} {res['alpha_pct']:>+8.2f} "
                  f"{res['cagr_pct']:>+8.2f} {res['sharpe']:>7.2f} "
                  f"{res['max_drawdown_pct']:>9.2f} {res['profit_factor']:>6.2f} "
                  f"{res['win_rate_pct']:>9.1f}")

        p = portfolio_result
        print("-" * 160)
        print(f"{'PORTFOLIO':<6} {p['total_trades']:>6} {'—':>10} {'—':>10} "
              f"{p['total_return_pct']:>+10.2f} {p['bnh_return_pct']:>+8.2f} "
              f"{p['alpha_pct']:>+8.2f} {p['cagr_pct']:>+8.2f} {p['sharpe']:>7.2f} "
              f"{p['max_drawdown_pct']:>9.2f} {p['profit_factor']:>6.2f} "
              f"{p['win_rate_pct']:>9.1f}")
        print("=" * 160)
...
class Trader:
    def __init__(self, config: Optional[dict] = None,
        ...
        self.event_log_dir = Path(self.cfg["daily_event_log_dir"])
        self.event_log_dir.mkdir(parents=True, exist_ok=True)
        self.daily_summary_state_file = Path(self.cfg["daily_summary_state_file"])
        self.ml_model  = None
        self.ml_scaler = None
        if self.cfg.get("use_ml", False):
            self._load_ml()
    ...
    def _daily_log_path(self) -> Path:
        """Returns die Tageslog-Datei fuer heute (YYYYMMDD-Format)."""
        today = datetime.now(pytz.UTC).strftime("%Y%m%d")
        return self.event_log_dir / f"events_{today}.log"

    def _load_daily_summary_state(self) -> dict:
        """Lädt den Zustand für einmal-pro-Tag Summary-Versand."""
        if self.daily_summary_state_file.exists():
            try:
                with open(self.daily_summary_state_file, "r", encoding="utf-8") as f:
                    return json.load(f)
            except Exception:
                return {"last_sent_date": None}
        return {"last_sent_date": None}

    def _save_daily_summary_state(self, date_str: str) -> None:
        """Merkt den letzten Versandtag der Tageszusammenfassung."""
        try:
            with open(self.daily_summary_state_file, "w", encoding="utf-8") as f:
                json.dump({"last_sent_date": date_str}, f, indent=2)
        except Exception as e:
            print(f"[WARN] Daily-Summary-State konnte nicht gespeichert werden: {e}")

    def _should_send_daily_summary(self, today: str) -> bool:
        """True, wenn Tageszusammenfassung im EOD-Zeitfenster liegt und heute noch nicht gesendet wurde."""
        # Nur am Tagesende (ET) senden
        now_et = datetime.now(pytz.UTC).astimezone(pytz.timezone("America/New_York"))
        send_after = self.cfg.get("daily_summary_send_after_et", "15:55")
        try:
            hh, mm = send_after.split(":")
            send_after_minutes = int(hh) * 60 + int(mm)
        except Exception:
            send_after_minutes = 15 * 60 + 55
        now_minutes = now_et.hour * 60 + now_et.minute
        if now_minutes < send_after_minutes:
            return False

        # Danach nur einmal pro Kalendertag senden
        if not self.cfg.get("daily_summary_once_per_day", True):
            return True
    ...
    def _log_event(self, level: str, event: str, symbol: str = "",
        ...
        level: "INFO", "WARN", "ORDER_FILLED", "ORDER_REJECTED",
               "CIRCUIT_BREAKER", "DEATH_CROSS", "MR_TARGET",
               "TRAILING_STOP", "DAILY_SUMMARY"
        """
        if details is None:
            details = {}
        try:
    ...
    def run_daily(self) -> dict:
        ...
        self._log_event("INFO", "Scan gestartet", details={
            "equity": round(equity, 2), "vix": round(vix, 1),
            "vix_reduce": vix_reduce, "open": list(open_positions.keys()),
        })
        ...
        for sym, pos in open_positions.items():
            ...
            symbol_status[sym] = (
                f"OPEN {pos['side'].upper()} {pos['qty']:.0f} @ {pos['entry']:.2f} "
        ...
        for sym in self.cfg["symbols"]:
            ...
            if df.empty:
                symbol_status[sym] = "NO_DATA (keine Kursdaten)"
                continue
            df = compute_indicators(df, self.cfg)
            sym_to_df[sym]  = df
            price_dict[sym] = float(df["Close"].iloc[-1])
            if sym not in symbol_status:
                symbol_status[sym] = f"DATA_OK {len(df)} Bars"
        ...
        if self._drawdown_active(equity):
            ...
            self._notify_and_log(
                "CIRCUIT_BREAKER", "Drawdown-Limit erreicht – keine neuen Trades",
                details={"dd_pct": round(dd_pct, 2), "peak": round(peak, 2),
                         "equity": round(equity, 2)},
                telegram_msg=(
                    f"[Botti v6] CIRCUIT BREAKER\n"
                    f"Drawdown {dd_pct:.1f}% (Limit {self.cfg['max_drawdown_pct']*100:.0f}%)\n"
            ...
            for sym in self.cfg["symbols"]:
                if sym in open_positions:
                    continue
                if sym in sym_to_df:
                    symbol_status[sym] = f"BLOCKED CircuitBreaker (DD {dd_pct:.1f}%)"
                else:
                    symbol_status.setdefault(sym, "NO_DATA (keine Kursdaten)")
        else:
            ...
            for sym, pos in list(open_positions.items()):
                ...
                if meta["strategy"] == "mr" and meta.get("mr_target") and \
                    ...
                    self._notify_and_log(
                        "MR_TARGET", "Mean-Reversion Ziel erreicht – Position geschlossen", sym,
                        details={"price": round(price, 2), "mr_target": meta.get("mr_target")},
                        telegram_msg=(
                            f"[Botti v6] {sym} MR TARGET erreicht\n"
                            f"Kurs {price:.2f} >= Ziel {meta.get('mr_target', 0):.2f} → geschlossen"
                        ),
                    )
                    continue
                ...
                if legs.get("stop"):
                    ...
                    if new_stop > legs["stop"]["stop_price"]:
                        old_stop = legs["stop"]["stop_price"]
                        self.alpaca.update_stop(legs["stop"]["id"], new_stop)
                        print(f"  {sym}: Trailing Stop "
                              f"{old_stop:.2f} → {new_stop:.2f}")
                        self._notify_and_log(
                            "TRAILING_STOP", "Stop-Loss angehoben", sym,
                            details={"old_stop": round(old_stop, 2), "new_stop": new_stop,
                                     "price": round(price, 2)},
                            telegram_msg=(
                                f"[Botti v6] {sym} Trailing Stop angehoben\n"
                                f"{old_stop:.2f} → {new_stop:.2f} (Kurs {price:.2f})"
                            ),
                        )
            ...
            for sym in self.cfg["symbols"]:
                ...
                if df is None or df.empty or len(df) < self.cfg["sma_long"] + 5:
                    if sym in sym_to_df:
                        symbol_status[sym] = f"SKIP zu wenig Historie ({len(df)} Bars)"
                    else:
                        symbol_status.setdefault(sym, "NO_DATA (keine Kursdaten)")
                    continue
                ...
                # Offene Alpaca-Position: Bracket-Order verwaltet Exit serverseitig
                if sym in open_positions:
                    if signal == "SELL":
                        # Death Cross: manuell schließen
                        self.alpaca.close_position(sym)
                        self.portfolio.del_meta(sym)
                        print(f"  {sym}: DEATH CROSS – Position geschlossen @ {price:.2f}")
                        self._notify_and_log(
                            "DEATH_CROSS", "Death Cross – Position manuell geschlossen", sym,
                            details={"price": round(price, 2)},
                            telegram_msg=(
                                f"[Botti v6] {sym} DEATH CROSS\n"
                                f"Position geschlossen @ {price:.2f}"
                            ),
                        )
                        symbol_status[sym] = f"CLOSED DEATH_CROSS @ {price:.2f}"
                    else:
                        symbol_status[sym] = f"OPEN HOLD ({reason or 'kein Exit-Signal'})"
                    continue

                if signal not in ("BUY", "BUY_MR"):
                    symbol_status[sym] = f"WATCH {signal} ({reason or 'kein Setup'})"
                    continue

                if atr_val == 0 or price == 0:
                    symbol_status[sym] = "SKIP ATR/Preis ungueltig"
                    continue

                # ── Multi-Timeframe-Filter (nur bei Trend-Signalen) ──────────
                if self.cfg.get("use_multi_timeframe", False) and signal == "BUY":
                    tf       = self.cfg.get("lower_timeframe", "15Min")
                    lk_days  = self.cfg.get("lower_tf_lookback_days", 10)
                    df_lower = self.alpaca.fetch_intraday_bars(sym, days=lk_days, tf=tf)
                    if df_lower.empty:
                        print(f"  {sym}: SKIP – keine {tf}-Daten")
                        symbol_status[sym] = f"SKIP keine {tf}-Daten"
                        continue
                    df_lower = compute_indicators(df_lower, self.cfg)
                    mtf_ok, mtf_reason = self.strategy.intraday_entry_confirmation(
                        df_lower, self.cfg)
                    if not mtf_ok:
                        print(f"  {sym}: SKIP – kein Lower-TF-Setup ({mtf_reason})")
                        symbol_status[sym] = f"SKIP Lower-TF ({mtf_reason})"
                        continue
                    reason += f" | {mtf_reason}"
                # ────────────────────────────────────────────────────────────
                ...
                if self._sector_count(sym, open_positions) >= self.cfg.get("max_per_sector", 2):
                    ...
                    symbol_status[sym] = "SKIP Sektor-Limit"
                    continue
                ...
                if self.cfg.get("use_ml", False):
                    ...
                    if prob < self.cfg["ml_prob_threshold"]:
                        ...
                        symbol_status[sym] = f"SKIP ML reject ({prob:.2f})"
                        continue
                ...
                order = self.alpaca.place_long_bracket(sym, shares,
                                                       ...
                                                       round(tp_price, 2))
                if order is None:
                    self._log_event("ORDER_REJECTED", "Order fehlgeschlagen", sym,
                                    {"shares": shares, "price": round(price, 2),
                                     "stop": round(stop_price, 2), "reason": reason})
                    symbol_status[sym] = "ORDER_REJECTED Broker"
                    continue
                ...
                symbol_status[sym] = f"BUY {shares} @ {price:.2f} [{strategy_t.upper()}]"
                self._notify_and_log(
                    "ORDER_FILLED", "BUY-Order ausgefuehrt", sym,
                    details={"shares": shares, "price": round(price, 2),
                             "stop": round(stop_price, 2), "tp": round(tp_price, 2),
                             "strategy": strategy_t, "reason": reason},
                    telegram_msg=(
                        f"[Botti v6] BUY {sym}\n"
                        f"{shares} Stk @ {price:.2f} [{strategy_t.upper()}]\n"
        ...
        print("\n  Symbol-Status:")
        for sym in self.cfg["symbols"]:
            print(f"  {sym:<6}: {symbol_status.get(sym, 'NO_STATUS')}")
        ...
        # Tagesabschluss-Benachrichtigung
        open_now  = list(self.alpaca.sync_positions().keys())
        trades_td = len([t for t in self.portfolio.data["trades"]
                         if t["time"].startswith(today)])
        bm_part   = (f"\n{self.cfg['benchmark']} BnH: {bm_ret:+.2f}%"
                     f"  Alpha: {(ret - bm_ret):+.2f}%" if bm_ret is not None else "")
        summary_details = {
            "equity": round(equity, 2),
        ...
        summary_msg = (
            f"[Botti v6] Tageszusammenfassung {today}\n"
            f"Equity: {equity:,.2f} {self.cfg['currency']} (Return {ret:+.2f}%){bm_part}\n"
            f"VIX: {vix:.1f}  |  Trades heute: {trades_td}\n"
            f"Positionen: {', '.join(open_now) if open_now else '(keine)'}"
        )

        if self._should_send_daily_summary(today):
            self._notify_and_log(
                "DAILY_SUMMARY", "Tagesabschluss",
                details=summary_details,
                telegram_msg=summary_msg,
            )
            self._save_daily_summary_state(today)
        else:
            # Vor EOD-Zeitfenster oder nach bereits gesendet: nur Log, kein Telegram.
            self._log_event(
                "DAILY_SUMMARY", "Tagesabschluss (kein Telegram in diesem Lauf)",
                details=summary_details,
            )
        return {
            "date":           today,
...
def main():
    ...
    if args.mode == "scan":
        ...
        if alpaca:
            ...
            send_telegram("Botti Trader v6: manueller EOD-Close ausgefuehrt")
        else:
